...
async def filter_rows(
    file_path: Annotated[str, Field(description="Absolute path to Excel file")],
    sheet_name: Annotated[str, Field(description="Worksheet name")],
    filters: Annotated[list[dict], Field(description="Filter conditions")],
        ...
        for row in range(2, (ws.max_row or 0) + 1):
            ...
            for f in filters:
                ...
                if operator == "==":
                    ...
                    result = value.lower() in str(cell_value).lower() if cell_value else False
            ...
            # Apply logic
            if logic.upper() == "AND":
                matches = all(filter_results)
            else:
                matches = any(filter_results)
...
async def group_by(
    ...
    agg_func: Annotated[str, Field(description="Aggregation function: sum, mean, median, count, min, max")],
) -> dict[str, Any]:
    ...
    try:
        ...
        for group_key, values in groups.items():
            ...
            result = {col: val for col, val in zip(columns, group_key)}
